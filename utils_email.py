# -*- coding: utf-8 -*-
# Arquivo: utils_email.py (Versão Completa e Corrigida)
...
def format_prazos(prazos_list: List[Dict[str, Any]]) -> str:
    ...
    for i, p_dict in enumerate(prazos_list, start=1):
        try:
            ...
            lines.append(f"{i}) Erro ao ler dados do prazo: {p_dict} - {e}"); lines.append(""); continue
        except Exception as e_gen:
            lines.append(f"{i}) Erro inesperado ao processar prazo: {p_dict} - {e_gen}"); lines.append(""); continue

        try: data_d_obj = datetime.strptime(p.data_d, '%Y-%m-%d').date(); data_d_str = data_d_obj.strftime('%d/%m/%Y')
        except: data_d_str = f"{p.data_d}(Inválido)" if p.data_d else "N/I"
        try: data_fatal_obj = datetime.strptime(p.data_fatal, '%Y-%m-%d').date(); data_fatal_str = data_fatal_obj.strftime('%d/%m/%Y')
        except: data_fatal_str = f"{p.data_fatal}(Inválido)" if p.data_fatal else "N/I"

        lines.append(f"   - Data D-: {data_d_str}"); lines.append(f"   - Data Fatal: {data_fatal_str}")
        obs = p.obs.strip();
        if obs: lines.append(f"   - Observações: {obs}")
        lines.append("")
    return "\n".join(lines).strip() # Remove blank line at the end
...
def format_pedidos_email(pedidos_data: List[PedidoData], tipo_decisao: str) -> str:
    ...
    linhas_email.append("-" * 30) # Separador

    # Determina quais instâncias mostrar com base no tipo de decisão
    ...
    for index, item in enumerate(pedidos_data, start=1):
        ...
        linhas_email.append("") # Linha em branco entre itens
...
def generate_email_body(**kwargs) -> tuple:
    ...
    if is_homologacao:
        ...
        if recurso_impeditivo_selecionado: # Modelo 3 ou 4
            ...
            if dep_anterior_valor > 0: # Modelo 1
                 log.debug("Execução - Modelo 1"); body_lines.append(f"Existem depósitos anteriores aprox. R$ {dep_anterior_valor:.2f}.");
                 if dep_anterior_detalhes: body_lines.append(f"  Detalhes: {dep_anterior_detalhes}")
                 body_lines.append("Vamos peticionar pela transferência atualizada."); pagamentos_extras = []
                 if calc_fgts > 0: pagamentos_extras.append("FGTS");
                 if calc_inss_emp > 0: pagamentos_extras.append("INSS Cota Empregado")
                 if pagamentos_extras: body_lines.append(f"Necessário pagamento de: {', '.join(pagamentos_extras)} em guia(s) própria(s)."); body_lines.append(f"Gentileza enviar os comprovantes até {data_d_pagamento_str}.")
                 else: body_lines.append("Acompanharemos a transferência."); body_lines.append("")
                ...
                if opcao_art_916 in ["Oferecer Opção Art. 916", "Cliente Optou por Art. 916"]:
                    ...
                    if opcao_art_916 == "Oferecer Opção Art. 916": body_lines.append(f"*Favor informar opção até [PRAZO RESPOSTA CLIENTE] e enviar comprovantes até {data_d_pagamento_str}.*")
                     ...
                     body_lines.append("PAGAMENTO NECESSÁRIO (Integral):"); body_lines.append(f"- Líquido Reclamante: R$ {valor_liq_reclamante:.2f}")
                     if calc_inss_emp > 0: body_lines.append(f"- INSS Empregado (Base): R$ {calc_inss_emp:.2f} (Guia e-Social)")
                     if calc_fgts > 0: body_lines.append(f"- FGTS: R$ {calc_fgts:.2f} (Guia Conta Vinculada)")
                     if calc_hon_suc > 0: body_lines.append(f"- Hon. Sucumbência: R$ {calc_hon_suc:.2f}")
                     if calc_hon_per > 0: body_lines.append(f"- Hon. Periciais: R$ {calc_hon_per:.2f}")
    ...
    # --- Lógica para Fase de Conhecimento ou Outras Decisões (Não Homologação) ---
    ...
    if prazos: # Tenta encontrar prazo principal
        prazo_acao_imediata = ""; # ... (lógica prazo_acao_imediata mantida) ...
        ...
        for p_dict in prazos:
            ...
            if prazo_acao_imediata and (prazo_acao_imediata.lower() in desc.lower()): prazo_principal = p; break
            except: continue
    if prazo_principal:
        try: # Formata datas do prazo principal
            data_d_cta = datetime.strptime(prazo_principal.data_d, '%Y-%m-%d').strftime('%d/%m/%Y')
            data_fatal_cta = datetime.strptime(prazo_principal.data_fatal, '%Y-%m-%d').strftime('%d/%m/%Y')
            desc_cta = prazo_principal.descricao
